...
namespace Common
{
	...
	static partial class HarmonyHelper // additional transpilers stuff to work with config
	{
		...
		[Obsolete]
		public static CIEnumerable _codeForChangeInstructionToConfigVar(string configVar, CodeInstruction ci = null)// => _codeForCfgVar(configVar);
		{
			FieldInfo varField = getCfgVarField(configVar); //FieldInfo varField = mainConfigField?.FieldType.field(configVar);
		...
		[Obsolete]
		public static CIEnumerable _codeForChangeConstToConfigVar<T, C>(T value, string configVar, ILGenerator ilg) where C: Component// => _codeForCfgVar<T, C>(value, configVar, ilg);
		{																											//	$"HarmonyHelper.changeConstToVar: injecting {value} => {configVar} ({typeof(C)})".logDbg();
			...
			yield return new CodeInstruction(OpCodes.Brtrue_S, lb1);
			yield return new CodeInstruction(LdcOpCode.get<T>(), value);
			yield return new CodeInstruction(OpCodes.Br_S, lb2);
